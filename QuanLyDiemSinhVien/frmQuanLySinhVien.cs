...

{
...

    {
        ...
        SELECT 
            ...
            l.TenLop AS Lop, -- Lấy TenLop và đặt tên cột là 'Lop'

            ROUND(AVG(d.DiemTB), 2) AS DiemTB, -- Tính Điểm TB trung bình và đặt tên là 'DiemTB'
        ...
        GROUP BY sv.MaSV, sv.HoTen, l.TenLop, sv.NgaySinh, sv.GioiTinh
    ...
    ";
            ...
            SqlCommandBuilder cmbSV = new SqlCommandBuilder(daSV);
            ...
            dvSinhVien = new DataView(ds.Tables["tblSV"]);
...

        {
            ...
            string boLoc =
                ...
                $"MaSV LIKE '%{tuKhoa}%' OR HoTen LIKE '%{tuKhoa}%'";
...

        {
...

            {
...

                {
                    ...
                    dgvMonHoc.DataSource = ds.Tables["tblMon"]; // Gán DataGridView với DataTable
            ...
            // Ẩn cột tiêu đề hàng để DataGridView trông sạch sẽ

            dgvMonHoc.RowHeadersVisible = false; // <<< BỔ SUNG DÒNG NÀY
...

        {
            ...
            cbMaMon1.ValueMember = "MaMon";





            cbMonHoc1.DataSource = dt.Copy();

            cbMonHoc1.DisplayMember = "TenMon";

            cbMonHoc1.ValueMember = "MaMon";
...

        {
            ...
            DataRow row = ds.Tables["tblMon"].NewRow();

            row["MaMon"] = cbMaMon1.SelectedItem;

            row["TenMon"] = cbMonHoc1.SelectedItem;

            row["SoTinChi"] = cbSTC.SelectedItem;

            ds.Tables["tblMon"].Rows.Add(row);
...

        {
            ...
            string tuKhoa = txtTim1.Text.Trim();
    ...
    ds.Tables["DiemThuan"].Columns["MaMon"]
        ...
        SELECT 
            ...
            SV.MaSV, SV.HoTen, L.TenLop, MH.MaMon, MH.TenMon, 

            D.DiemGK, D.DiemCK, D.DiemTB,

            -- Cột Điểm Tổng Kết (GPA)

            CASE 

                WHEN TS.TongSoTinChi > 0 THEN ROUND(TS.TongDiemCoTrongSo / TS.TongSoTinChi, 2) 

                ELSE NULL 

            END AS DiemTongKet,

            -- Cột Xét Học Bổng (Học bổng >= 8.0 VÀ không rớt môn)

            CASE 

                WHEN (TS.TongDiemCoTrongSo / TS.TongSoTinChi) >= 8.0 

                 AND TS.TinhTrangHocTap = N'Đủ điều kiện' 
        ...
        -- Subquery TS tính tổng điểm, tổng tín chỉ, và kiểm tra rớt môn
        ...
        LEFT JOIN (
            ...
            SELECT
                ...
                SUM(d_sub.DiemTB * mh_sub.SoTinChi) AS TongDiemCoTrongSo,

                SUM(mh_sub.SoTinChi) AS TongSoTinChi,

                -- Kiểm tra xem có môn nào Điểm TB < 5.0 không

                CASE WHEN MIN(d_sub.DiemTB) >= 5 THEN N'Đủ điều kiện' ELSE N'Rớt môn' END AS TinhTrangHocTap

            FROM Diem d_sub

            JOIN MonHoc mh_sub ON d_sub.MaMon = mh_sub.MaMon

            GROUP BY d_sub.MaSV

        ) AS TS ON SV.MaSV = TS.MaSV";
            ...
            // Gán nguồn dữ liệu cho DataGridView (dgvTraCuu)

            dgvTraCuu.DataSource = ds.Tables["tblTraCuu"];
            ...
            dgvTraCuu.Columns["MaSV"].HeaderText = "Mã SV";

            dgvTraCuu.Columns["HoTen"].HeaderText = "Họ và Tên";

            dgvTraCuu.Columns["TenLop"].HeaderText = "Lớp";

            dgvTraCuu.Columns["MaMon"].HeaderText = "Mã Môn";

            dgvTraCuu.Columns["TenMon"].HeaderText = "Tên Môn";

            dgvTraCuu.Columns["DiemGK"].HeaderText = "Điểm GK";

            dgvTraCuu.Columns["DiemCK"].HeaderText = "Điểm CK";

            dgvTraCuu.Columns["DiemTB"].HeaderText = "Điểm TB";

            dgvTraCuu.Columns["DiemTongKet"].HeaderText = "Điểm Tổng Kết";

            dgvTraCuu.Columns["XetHocBong"].HeaderText = "Xét Học Bổng";
...

        {
...

            {
                ...
                LoadTraCuuData(); // Tải lại toàn bộ dữ liệu
...

            {
                ...
                // Truy vấn SQL đầy đủ (giống LoadTraCuuData) nhưng có thêm điều kiện WHERE cho tìm kiếm
            ...
            WHERE SV.MaSV LIKE @kw

               OR SV.HoTen LIKE @kw

               OR L.TenLop LIKE @kw

               OR MH.TenMon LIKE @kw"; // Điều kiện tìm kiếm
...

        {
            ...
            // Ngăn chặn chạy khi đang ở chế độ Thêm/Sửa
            ...
            // Xử lý Ngày Sinh (DATE)

            if (row.Cells["NgaySinh"].Value != DBNull.Value && row.Cells["NgaySinh"].Value != null)

            {

                // Chuyển đổi từ giá trị trong DataGridView (thường là DateTime)
            ...
            // Xử lý Giới Tính (Nvarchar(50) trong SQL, so sánh với giá trị 'Nam'/'Nu' trong SQL)

            if (row.Cells["GioiTinh"].Value != DBNull.Value && row.Cells["GioiTinh"].Value != null)

            {

                // Kiểm tra giá trị trong cột GioiTinh của DataGridView/DataTable

                string gioiTinh = row.Cells["GioiTinh"].Value.ToString();



                // Giả sử GioiTinh trong database là 'Nam' hoặc 'Nu'

                chkNam.Checked = (gioiTinh == "Nam");

                chkNu.Checked = (gioiTinh == "Nu");
...

            {
                ...
                cbLop.SelectedValue = row.Cells["MaLop"].Value; // LỖI TÊN CỘT
...

        {
            ...
            if (!btnThem1.Enabled || !btnSua1.Enabled) return;



            if (e.RowIndex < 0 || e.RowIndex >= dgvMonHoc.RowCount) return;
            ...
            // --- Đổ dữ liệu xuống các ô nhập liệu ---



            // Mã Môn

            cbMaMon1.Text = row.Cells["MaMon"].Value?.ToString() ?? "";



            // Tên Môn

            cbMonHoc1.Text = row.Cells["TenMon"].Value?.ToString() ?? "";



            // Số Tín Chỉ

            if (row.Cells["SoTinChi"].Value != DBNull.Value && row.Cells["SoTinChi"].Value != null)

            {

                // Gán giá trị Số Tín Chỉ (Số Tín Chỉ thường là ComboBox hoặc TextBox)

                cbSTC.Text = row.Cells["SoTinChi"].Value.ToString();
            ...
            // Khóa lại các nút Thêm/Sửa/Xóa sau khi click
